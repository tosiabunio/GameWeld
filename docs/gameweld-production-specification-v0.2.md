...
| Workboard | Execute tasks, view current item scope, and identify out-of-scope work. Archived Workboards are listed under the board and open under an address of their own, showing the cards as they stood with nothing on them changeable. The scope is not a panel of its own: the count of items in scope, in the board's header, opens it, with each item's state, its Breakdown, the way out of the scope, and what can be added next. The count is marked while an item waits for review. |
| Task detail | Edit task content, its checklist, assignment, comments, links, and attachments. Show parent and current placement. It opens as a window over the screen the task was opened from, by a click anywhere on the task's card, and closes back onto that screen with its close button, Escape, or a click outside; unsaved text is not discarded without asking. The task keeps an address of its own, which opened directly shows the window over the Workboard the task is on, or over its item's Breakdown. |
| Requests | Lightweight list or panel for pending out-of-scope work requests. |
| Notifications | A bell in the top bar, on every screen, across the member's projects. It counts and opens two lists. "Waiting for you" is read from the state itself and stays until someone decides: placement requests, for Game Directors, and items ready for review, for those who may accept. "Notifications" are events: a task given to the member or taken from them, a comment on their task or in a conversation they joined, a request made (to Directors) or decided (to the requester, also when the system settled it), an item ready for review (to those who may accept), and an item accepted or sent back (to those whose tasks it holds). Nobody is notified of what they did themselves, or of anything in a project they have left. Opening a notification opens what it is about and marks it read. |
| Project settings | Membership, role assignments, completion policy, board scope limit, and board management. A Game Director also takes the project's data out and brings a Trello board in. The export is one JSON document with everything in the project (settings, members, labels, items, tasks with their checklists, comments and links, Workboards with their placements, requests, and the whole activity history; attachments by name, the files stay in storage), or the tasks alone as CSV, one per row. The import reads Trello's JSON export in one of two ways, because a Trello card is a task to some teams and a feature to others: each list becomes a backlog item holding its cards as tasks, with their labels, date, checklists, comments, and links; or each card becomes a backlog item whose checklist entries become its tasks. Archived lists and cards are left out, a label that names a kind of work decides a task's category, and everything arrives unplaced, under Should Have. |
| Quick open | Cmd+K (Ctrl+K elsewhere), or its button in the top bar, on every screen: a few letters find a task or a backlog item of the open project by title or description, a section of it, or another project. A task opens in its window over the screen in view. |

The Backlog and the Workboard filter their cards by part of a title, an assignee (or unassigned), and a task category; the Workboard adds one backlog item and "out of scope only". Search and the filters wait behind two icons beside the display options, so the top of the screen stays quiet until they are wanted; the filter icon counts the filters that are on. The search field comes out beside its icon. Cards that do not match are hidden and each lane shows how many of its cards match. While the search or a filter hides cards, and only then, a line above the lanes says how many cards show in all and clears the filters. A backlog item has no assignee or task category of its own: it matches when one of its tasks that are not deleted, finished or not, has that assignee and that category at once. The filters are the viewer's own, shared by the two screens, and last for the visit. While they hide cards, cards cannot be reordered within a lane; a card can still be moved to another lane, where it joins the end.
...
The interface speaks English and Polish. The language is the viewer's own, kept in the browser, chosen on the profile page, and the browser's language the first time. English is the source: a text that has no translation yet shows in English. What users write, and what the server says when it refuses something, is not translated.

On a phone a swipe over cards scrolls the page, a short hold picks a card up, and a tap opens it; with a mouse a card drags once it has moved a little, so that a click stays a click.
