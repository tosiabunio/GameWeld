...
| Workboard | Execute tasks, view current item scope, and identify out-of-scope work. The scope is not a panel of its own: the count of items in scope, in the board's header, opens it, with each item's state, its Breakdown, the way out of the scope, and what can be added next. The count is marked while an item waits for review. |
