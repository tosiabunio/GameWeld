...
export const labelRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get(base, projectRoute('project.view'), async (req) =>
    fetchLabels(db, req.access!.project.id),
  );
  ...
  app.post(base, projectRoute('task.work'), async (req, reply) => {
    ...
    try {
      ...
      if (isDuplicate(e)) throw conflict(`There is already a label called “${parsed.data.name}”.`);
      throw e;
    }
    return reply.status(201).send(await fetchLabels(db, projectId));
  });

  app.patch(`${base}/:labelId`, projectRoute('backlog.manage'), async (req) => {
    const parsed = labelSchema.safeParse(req.body);
    if (!parsed.success) throw badRequest('Invalid label', parsed.error.flatten());
    const projectId = req.access!.project.id;
  ...
  app.delete(`${base}/:labelId`, projectRoute('backlog.manage'), async (req) => {
    const projectId = req.access!.project.id;
    const { labelId } = req.params as { labelId: string };
