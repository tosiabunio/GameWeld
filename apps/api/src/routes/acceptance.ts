...
export const acceptanceRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post(
    `${base}/accept`,
    projectRoute('item.accept', {
      id: 'acceptItem',
      summary: 'Accept an item that is Ready for Review as Done',
      description:
        'The acceptance stands until a task of the item is reopened or added, which returns the item to Open.',
      body: acceptSchema.optional(),
  ...
  app.post(
    ...
    projectRoute('item.accept', {
      id: 'rejectItem',
      summary: 'Reject an item that is Ready for Review',
      description: 'The note is posted as a rejection comment on the item, which answers with it.',
      body: rejectSchema,
